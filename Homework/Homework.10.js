...
/**Requirement:
 * 
Write a function named calculateTotalPrice1() which takes an object 
of some shopping items with their quantities as key-value pairs and 
...
const calculateTotalPrice2 = cart => {
    ...
    for(const item in cart) {
    if(item === 'apple' && cart[item] > 1) total += prices[item] * (0.5 * cart[item] + Math.floor(cart[item] / 2) * 0.5);
...
/*
Write a function named isArmstrong() which takes a number argument and returns 
true if given number is armstrong, return false otherwise.​
Note: An armstrong number is a number that is equal to the sum of its own digits
 raised to the power of the number of digits. In other words, an n-digit number is an Armstrong 
 number if the sum of its digits, each raised to the nth power, is equal to the number itself.
...
of digits in 153) is equal to the original number, which means 153 is an armstrong number.
Examples:
...
const isArmstrong = (num) => {
     ...
     let digits = num.toString().split('').map((n) => Number(n));
...
/*
Requirement:
Write a function named doubleOrTriple() which takes an array of numbers 
as argument and a boolean value. It will return the array elements doubled if true 
...
*/
function doubleOrTriple(arr, isTrue) {
    let result = [];
